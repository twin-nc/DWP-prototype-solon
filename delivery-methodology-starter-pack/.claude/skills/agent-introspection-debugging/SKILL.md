...
**Scope: agent-level failures only.** This skill diagnoses failures in how an agent reasons, uses tools, or manages context. If the failure is a Maven build error on your machine, use `java-build-resolver`. If it is a red check on a GitHub Actions PR, use `ci-failure-root-cause-fixer`. If it is a post-deploy regression on a running service, use `canary-watch`. If it is a frontend state-sequence bug, use `click-path-audit`.
...
- The failure is a Maven compilation, dependency, Flyway, or autoconfiguration error — use `java-build-resolver`
- The failure is a red CI check on a pull request — use `ci-failure-root-cause-fixer`
- The failure is an intermittently failing test — use `flaky-test-triage`
- The failure is a Docker Compose container not starting — use `docker-compose-debugger`
- The failure is a regression detected after deploy — use `canary-watch`
- The failure is a frontend button or state-sequence bug — use `click-path-audit`
