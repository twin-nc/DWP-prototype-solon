...
- **Escalate / hand off when:**
  - to `TEST-DESIGNER.md` when broader verification strategy or test-layer mix is unclear
  - to `BUSINESS-ANALYST.md` when behavior, actor expectations, or scope boundaries are unclear
  - to `SOLUTION-ARCHITECT.md` when ownership boundaries, state transitions, or orchestration responsibilities are ambiguous
  - to `TAX-DOMAIN-EXPERT.md` when expected workflow outcomes depend on tax or policy interpretation
  - to `DEVOPS-RELEASE-ENGINEER.md` when environment, observability, or release-evidence implications dominate
  - to `DESIGN-CRITIC.md` when the source hides contradictory assumptions or risky cross-boundary behavior
