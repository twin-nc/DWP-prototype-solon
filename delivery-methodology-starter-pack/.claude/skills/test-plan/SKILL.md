...
- **Escalate / hand off when:**
  - to `BUSINESS-ANALYST.md` when behavior, scope, or expected outcomes are unclear
  - to `SOLUTION-ARCHITECT.md` when state, ownership boundaries, or lifecycle behavior are ambiguous
  - to `TAX-DOMAIN-EXPERT.md` when policy or tax interpretation determines expected results
  - to `DEVOPS-RELEASE-ENGINEER.md` when release gates, CI expectations, or environment validation become central
  - to `DESIGN-CRITIC.md` when the slice appears to hide contradictory assumptions or risky blind spots
