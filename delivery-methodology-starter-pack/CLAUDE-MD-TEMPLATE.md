...
**COTS note:** This is a configuration and integration project, not a greenfield build. The COTS platform handles its own internal database, workflow engine, and UI. Netcompany builds and owns the integration adapters, configuration scripts, and any bespoke extension services. Standards apply differently to each layer — see `docs/project-foundation/standards/deviations/DEV-001-cots-scope.md` before starting any implementation work.
