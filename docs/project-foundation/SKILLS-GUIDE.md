...
### Merge candidate: `standards-governance-reviewer` + `documentation-authority-resolver`

These skills were originally flagged for merging because both address source-of-truth conflicts. On review, the split is defensible: `documentation-authority-resolver` answers "which source wins" (authority hierarchy question), while `standards-governance-reviewer` answers "are we compliant with the standards pack" (compliance judgment question). They cross-reference each other correctly in their invocation boundaries. **Decision: keep separate unless a real invocation friction pattern emerges.** Reassess at the next skills audit.
...
## Skills Currently Missing — Status for DWP DCMS

These skills were identified from project retrospective analysis. Status column reflects their relevance to this project specifically.

### `i18n-completeness-checker`
**Status for DWP DCMS:** Relevant — Welsh (UI.8) is a tender requirement. **Create when frontend build begins.** The frontend-builder guardrail covers day-to-day discipline; this skill becomes necessary as a CI gate once string volume grows.
...
**Status for DWP DCMS:** Not applicable currently — single UK jurisdiction, single DWP client. Revisit only if multi-tenancy or multi-jurisdiction scope is added.
...
**Status for DWP DCMS:** High priority — `EMERGENCY-BYPASS-PROCEDURE.md` exists and is referenced. Without this skill, hotfix governance depends on developers remembering procedure under time pressure. **Create before first remote deployment.**
...
**Status for DWP DCMS:** Relevant — the backend has defined state machines for account lifecycle, repayment plan lifecycle, and breathing space. When frontend builds workflow screens for these, parity is a real risk. **Embed in Frontend Builder's pre-merge checklist (already added to `frontend-builder.md`).** Create the full skill before first debt workflow screen is built.
