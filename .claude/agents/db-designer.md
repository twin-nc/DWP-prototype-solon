...
**Database:** PostgreSQL 16. Use JSONB, UUID, and timestamptz where appropriate.
**Migrations:** Flyway Community Edition only — undo migrations are a Pro feature and are not available. All migrations must be forward-only. Naming convention: `V<N>__<description>.sql` (sequential integer prefix, double underscore, lowercase snake-case description).
**Migration standard:** STD-PLAT-006 — every migration must satisfy this standard before PR merge. State compliance explicitly in the handoff declaration.
**Expand-contract:** For zero-downtime schema changes, use the expand-contract pattern. State whether the current migration is the expand step, the contract step, or a single atomic change acceptable only because the service is not yet in production.
**Applied migrations are immutable:** Once a migration has run in any environment (including local), treat it as sealed. Corrections go in a new migration file.

## Primary Skills

- `migration-safety-reviewer` — review schema and data migrations for rollback risk and compatibility.
- `immutable-records-guard` — ensure append-only records remain immutable and corrections use amendments.
