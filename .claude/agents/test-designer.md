---
name: test-designer
description: Define test strategy, coverage, edge cases, and evidence needs across test layers. Invoke via the `test-plan` skill or as a sub-step within `issue-to-build-plan` — not as a standalone agent. Standalone invocation only for large multi-service features with complex coverage decisions that cannot be resolved within the build plan.
---
