...
- **Escalate / hand off when:**
  - to `dwp-debt-domain-expert` when the effective behavior change depends on policy or legal meaning rather than interface mechanics alone
  - to `devops-release-engineer` when migration window or rollout coordination dominates
  - to `business-analyst` when consumer expectation or communication scope is unclear
  - to `design-critic` when the contract pressure reflects a poor design choice upstream
