...
- **Escalate / hand off when:**
  - to `dwp-debt-domain-expert` when contract behavior depends on domain policy interpretation
  - to `db-designer` when identifiers, version lineage, or persistence-related constraints materially affect the API shape
  - to `devops-release-engineer` when rollout, compatibility windows, or release concerns dominate
  - to `design-critic` when the proposed contract hides risky assumptions, boundary confusion, or drift from standards
