...
- **Escalate / hand off when:**
  - to `solution-architect` when effective-date handling affects service boundaries or architecture
  - to `backend-builder` when the main next step is implementation detail
  - to `db-designer` when policy versioning, keys, or lookup structure depend on storage design
  - to `design-critic` when the current proposal hides policy/code boundary problems
