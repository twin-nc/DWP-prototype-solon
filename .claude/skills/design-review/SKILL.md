...
- **Escalate / hand off when:**
  - to `solution-architect` when the issue becomes one of defining the target architecture rather than critiquing it
  - to `dwp-debt-domain-expert` when the design depends on domain policy interpretation
  - to `db-designer` when storage model, version lineage, or migration strategy becomes central
  - to `devops-release-engineer` when rollout, release, operability, or rollback strategy is central
  - to `code-reviewer` when the question is really about implementation details in a diff
