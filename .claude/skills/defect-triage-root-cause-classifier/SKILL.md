...
- **Escalate / hand off when:**
  - to `code-reviewer` when the defect clearly maps to a specific code change
  - to `solution-architect` when the defect reflects boundary or state-model flaws
  - to `db-designer` when data lineage or migration is implicated
  - to `dwp-debt-domain-expert` when the expected outcome itself may be policy-ambiguous
