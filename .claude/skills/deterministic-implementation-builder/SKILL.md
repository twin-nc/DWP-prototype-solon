...
- **Escalate / hand off when:**
  - to `dwp-debt-domain-expert` when deterministic behavior depends on policy interpretation
  - to `solution-architect` when deterministic guarantees cross service boundaries
  - to `db-designer` when ordering, lineage, or replay metadata depend on persistence strategy
  - to `test-builder` when the main next step is generating deterministic test coverage
  - to `design-critic` when the current approach hides nondeterministic assumptions
