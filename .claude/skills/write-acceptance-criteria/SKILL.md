...
- Product-adjacent roles represented by `delivery-designer`
- `solution-architect`
- `test-designer`
- `test-builder`
...
- **Escalate / hand off when:**
  - to `dwp-debt-domain-expert` when tax or policy interpretation determines correct acceptance behavior
  - to `solution-architect` when state behavior, ownership boundaries, or lifecycle semantics are ambiguous
  - to `delivery-designer` when the slice spans multiple domains or needs broader solution framing
  - to `test-designer` when the acceptance criteria need scenario decomposition or structured test-design support
  - to `design-critic` when the slice appears to hide conflicting assumptions or cross-lane issues
