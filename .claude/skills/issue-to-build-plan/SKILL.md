...
- **Escalate / hand off when:**
  - to `solution-architect` when the issue crosses service, boundary, or state-model concerns
  - to `dwp-debt-domain-expert` when policy or tax interpretation affects scope or expected behavior
  - to `db-designer` when data lineage, immutable history, or migration strategy becomes central
  - to `devops-release-engineer` when release evidence, CI gates, rollout, or environment concerns become central
  - to `design-critic` when the issue implies contested architectural assumptions or lane-boundary problems
