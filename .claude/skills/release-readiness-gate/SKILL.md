...
- **Escalate / hand off when:**
  - to `solution-architect` when unresolved architecture or boundary concerns are gating release
  - to `db-designer` when migration safety is the main blocker
  - to `business-analyst` when expected behavior or scope acceptance is unclear
  - to `design-critic` when the evidence points to unsafe design assumptions
