...
- **Escalate / hand off when:**
  - to `dwp-debt-domain-expert` when amendment legality or business meaning is unclear
  - to `solution-architect` when append-only behavior affects service boundaries or state projections
  - to `backend-builder` when the main next step is implementation changes
  - to `design-critic` when the design assumes unsafe mutation shortcuts
