...
- **Escalate / hand off when:**
  - to `solution-architect` when mappings depend on unresolved design or contract decisions
  - to `devops-release-engineer` when evidence run IDs or release metadata are the missing link
  - to `design-critic` when the change lacks enough clarity to map confidently
