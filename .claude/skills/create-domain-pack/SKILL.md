...
- **Escalate / hand off when:**
  - to `dwp-debt-domain-expert` when policy or tax interpretation affects entities, lifecycle rules, or invariants
  - to `db-designer` when immutable history, version lineage, or projection/storage strategy becomes central
  - to `delivery-designer` when domain decisions affect wider solution boundaries
  - to `design-critic` when ownership, layering, or architecture assumptions are contested
