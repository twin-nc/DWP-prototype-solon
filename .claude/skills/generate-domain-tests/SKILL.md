...
- **Escalate / hand off when:**
  - to `test-designer` when broader verification strategy or test-layer selection is unclear
  - to `solution-architect` when state behavior, invariants, or ownership boundaries are ambiguous
  - to `dwp-debt-domain-expert` when expected outcomes depend on domain policy interpretation
  - to `backend-builder` when domain behavior depends on implementation structure that must be clarified
  - to `design-critic` when the source contains contradictory assumptions or unstable modeling choices
